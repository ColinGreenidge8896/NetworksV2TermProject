...
public:
	...
	void SetCmd(CmdType cmd)
	{
		...
		case DRIVE:
		{
			CmdPack.header.drive = 1;
			CmdPack.header.length = headerSize + 3 + 1;
			break;
		}
		case STATUS:
		{
			CmdPack.header.status = 1;
			CmdPack.header.length = headerSize + 1;
			break;
		}

		case SLEEP:
		{
			CmdPack.header.sleep = 1;
			CmdPack.header.length = headerSize + 1;
			break;
		}
		case ACK:
	...
	CmdType GetCmd()
	{
		if (CmdPack.header.drive)
			return DRIVE;
		if (CmdPack.header.sleep)
			return SLEEP;
		if (CmdPack.header.status)
			return STATUS;
		return ACK;
	}
